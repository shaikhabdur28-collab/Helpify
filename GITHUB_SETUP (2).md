...
If you want the AI chatbot to work on the website, add a `.env` file before building:

```bash
VITE_GEMINI_API_KEY=your_gemini_api_key
```

Then run a normal build and deploy.

Important: because GitHub Pages is static, this API key will be visible in the published site bundle. For a private production chatbot, move the Gemini call to a backend later.
